...
def plot_heatmaps(runid):
    ...
    l_param = params[4] if len(params) >= 5 else None  # e.g., meas_voltage_Lockin1
    ...
    if abs(y_max) > abs(y_min) and y_max > 0:
        scan_direction = 'Ic+'  # 正向掃描
    elif abs(y_min) > abs(y_max) and y_min < 0:
        scan_direction = 'Ic-'  # 負向掃描

    for name, group in df.groupby(x_param):
        analysis = analyze_iv_curve(group, x_param=y_param, y_param=v_param, dvdi_param=dvdi_param)
        if scan_direction == 'Ic+':
            ic_val = analysis['Ic'] if analysis['Ic'] > 0 else np.nan
        elif scan_direction == 'Ic-':
            ic_val = analysis['Ir'] if analysis['Ir'] < 0 else np.nan
        else:
            ic_val = analysis['Ic']  # fallback
        ic_values.append({'param_value': name, 'Ic': ic_val})
    ...
    fig3 = go.Figure(data=go.Scatter(
        ...
        y=ic_df['Ic'],
        mode='lines+markers',
        name='Ic+' if scan_direction == 'Ic+' else 'Ic-',
        marker=dict(size=5),
        line=dict(width=2)
    ))
    fig3.update_layout(
        title=f"#{runid}|{exp_name}|{scan_direction or 'Ic+'}|{sample_name}",
        xaxis_title=f"{x_param} ({param_units.get(x_param, '')})",
        yaxis_title=f"{scan_direction or 'Ic'} ({param_units.get(y_param, '')})"
    )
